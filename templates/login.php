...
<center>
    ...
    <form method='POST' action='/index.php?page=login'>
        <?php if($_POST['login'] == 'admin' && $_POST['pass'] == 'admin'):?>
            <?php $_SESSION['auth'] = 'admin'; // todo что за авторизация в шаблоне??? такого быть не должно?>
        <?php endif?>
        <?php if($_SESSION['auth'] == 'admin' || $_SESSION['auth'] == 'user'): //todo не использовать $_SESSION в шаблоне ?>
            <div style="width:120px; text-align:center;">
                <input name='submit1' type='submit' value='Выход'>
            </div>
        <?php endif?>
        <?php if($_SESSION['auth'] != 'user' && $_SESSION['auth'] != 'admin'): //todo не использовать $_SESSION в шаблоне ?>
    ...
    <?if($_SESSION['auth'] == 'user'): //todo не использовать $_SESSION в шаблоне  ?>
        <br><br>
        <font color="red" face="Zapf Chancery, cursive"><h2>Вы авторизированы как USER</h2></font>
    <?endif?>
    <?if($_SESSION['auth'] == 'admin'): //todo не использовать $_SESSION в шаблоне ?>
        <br><br>
